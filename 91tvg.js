...
[task_local]
#91TVG签到
30 8,12 * * * 91tvg.js, tag=91TVG签到, enabled=true

 */
//在环境变量中新建两个变量，名称分别为 TVG_COOKIE 和 TVG_FORMHASH,值填写对应的抓包内容，就可以使用了
...
function qiandao() {
	return new Promise(resolve => {
		$.post(taskPostUrl('https://www.91tvg.com/plugin.php?id=dc_signin:sign&inajax=1', 'formhash=' + formhash + '&signsubmit=yes&handlekey=signin&emotid=1&referer=https://www.91tvg.com/plugin.php?id=dc_signin&content=' + todaysay ), async (err, resp, data) => {
			try {
                

                //dc_signin', '签到成功~随机奖励经验值16'
...
function taskPostUrl(post_url, post_body) {
	return {
		...
		headers: {
			...
			"referer": "https://www.91tvg.com/plugin.php?id=dc_signin"
		}
	}
}
